...
class VirtualSheet(Sheet):
    ...
    def remove_virtual_vertices(self):
        ...
        while short.size > 0:
            # before = self.check_all_edge_order()
            ...
            collapse_edge(self, short[0], allow_two_sided=False, reindex=True)
            short = self.edge_df[self.edge_df["length"] < self.minimal_bond_length].index.to_numpy()
            if short.size > 0:
                short = short[self.is_virtual_edge(short)]
            np.random.shuffle(short)
            # after = self.check_all_edge_order()
...
class InnerEarModel:
    ...
    def __init__(self, sheet, tension=None, repulsion=None, repulsion_distance=None, repulsion_exp=7,
        ...
        self.initialize_random_notch_delta(random_sensitivity)
    ...
    def get_model(only_differentiation=False, add_random_forces=False):
        if only_differentiation:
            ...
            model_factory([LineTension, FaceContractility, FaceAreaElasticity, FaceRepulsion, RandomAffector])
        return model
        ...
        def division(sheet, manager):
            ...
            for cell_id, row in sheet.face_df.iterrows():
                if row.area > crit_area and row.type == 0:
                    # Do division
                    daughter = cell_division(sheet, cell_id, geom)[0]
                    # Update the topology
                    sheet.get_opposite()
    ...
    def initialize_random_notch_delta(self, random_sensitivity=False):
...
if __name__ == "__main__":
    initial_sheet_name = "uniform_sensitivity"
    name = "notch_inhibition"
    ...
    if os.path.isfile("%s.hf5" % initial_sheet_name):
        ...
        sheet = VirtualSheet.planar_sheet_2d(
            ...
            ny=21,  # approximae number of cells along the y axis
        ...
        sheet.set_minimal_bond_length(0.02)  # was 0.05
