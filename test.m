...
% Generate different values of f for each cell from a normal distribution
mean_f = params.mean_f; % Mean of the normal distribution
std_f = params.std_f;   % Standard deviation of the normal distribution
params.f = normrnd(mean_f, std_f, [k, 1]);
...
dD = nu * (betaD.*params.f.^h./(params.f.^h + R.^h)-D); 
...
params.P=10;        % number of cells per column
params.Q=10;        % number of columns - MUST BE EVEN
params.mean_f=6;    % Mean of the normal distribution for f
params.std_f=1;     % Standard deviation of the normal distribution for f
params.g=1;
