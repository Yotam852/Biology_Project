# A code that runs LI for multiple <k> values and preforms a neighbor analysis for each run
...
def multicell_LI(mean_k):
    ...
    # Export the DataFrame to a CSV file with mean_k in the filename
    filename = f'neighbors_analysis_mean_k_{mean_k}.csv'
    neighbors_df.to_csv(filename, index=False)
...
def defaultparams(mean_k):
    return {
        ...
        'P': 10,
        'Q': 10,
        'k_mean': mean_k,  # Mean of k distribution
...
if __name__ == "__main__":
    ...
    for mean_k in low_means:
        ...
        yout, tout, params, neighbors_df = multicell_LI(mean_k)
