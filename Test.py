...
def run_simulations():
    ...
    for i, beta0 in enumerate(beta0_values):
        for j, f in enumerate(f_values):
            ...
            if ratio > 2:
                print(f"Patterning detected at beta0: {beta0}, f: {f}, D_max: {D_max}, D_min: {D_min}, ratio: {ratio}")
    ...
    X, Y = np.meshgrid(f_values, beta0_values)
    plt.figure()
    plt.style.use(['science', 'notebook', 'grid'])
    contour = plt.contourf(X, Y, D_ratios_contour, levels=np.linspace(0, np.max(D_ratios_contour), 100), cmap='viridis')
    plt.colorbar(contour)
    plt.xlabel('f [a.u]')
    plt.ylabel('beta0 [a.u]')
    plt.title('Contour plot of $D_{max} / D_{min}$')
    plt.show()
